    ...
    public class CodeValueService : CodeValueProtoService.CodeValueProtoServiceBase
    {
        ...
        public override async Task<Response> GetByCodeName(GetByCodeNameRequest request, ServerCallContext context)
        {
            try
            {
                ...
                var structData = ConvertToStruct(result);

                Response.Data = structData;
                Response.IsSuccess = true;
        ...
        private Google.Protobuf.WellKnownTypes.Struct ConvertToStruct(List<Dictionary<string, object>?> data)
        {
            var res = new Google.Protobuf.WellKnownTypes.Struct();
            foreach (var dictionary in data)
            {
                if (dictionary != null)
                    foreach (var keyValuePair in dictionary)
                    {
                        res.Fields[keyValuePair.Key] =
                            Google.Protobuf.WellKnownTypes.Value.ForString(keyValuePair.Value.ToString());
                    }
            }
            return res;
        }
